    ...
    <div class="container">
        ...
        <?php
            ...
            // connexion a la base de donnees
             ...
             $user_name_query = "SELECT Login FROM brand WHERE id_br = ?";
             $user_id_statement = $pdo->prepare($user_name_query);
             $user_id_statement->execute([$user_id]);
             $user_id_result = $user_id_statement->fetch();
             $user_id = $user_id_result['Login'];


            /*La requête sélectionne les marque où la colonne "Brand_Login" est égale à l'identifiant d'utilisateur stocké dans la variable $user_id */
            $requete = "SELECT Brand, influencer, amount, duration, Terme, Signature_inf, Signature_Brand, Brand_Login, Influencer_Login FROM partenariat WHERE Brand_Login = ?";
            //La requête est préparée avec la méthode prepare() de l'objet PDO
            $result = $pdo->prepare($requete);
            //elle est exécutée avec la méthode execute() en lui passant un tableau contenant la valeur de la variable $user_id
